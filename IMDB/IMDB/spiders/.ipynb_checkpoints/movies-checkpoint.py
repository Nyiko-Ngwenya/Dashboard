    ...
    def parse(self,response):
        for movies in response.xpath('//div[@class="lister-item-content"]'):
            yield{
                 ...
                 'Votes' : movies.xpath('.//p[@class="sort-num_votes-visible"]/span[2]/text()').extract_first(),
                 'Gross Amount' : movies.xpath('.//p[@class="sort-num_votes-visible"]/span[5]/text()').extract_first(),
                 'Rating' : movies.xpath('.//div[@class="inline-block ratings-imdb-rating"]/strong/text()').extract_first(),
                 'Metascore' : movies.xpath('.//div[@class="inline-block ratings-metascore"]/span/text()').extract_first()
            }
